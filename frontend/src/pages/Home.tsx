...
import { Star, ArrowRight, BookOpen, TrendingUp, Award, ShoppingCart, Heart, Sparkles, Loader2, X } from 'lucide-react';
...
const CATEGORIES = [
  { name: "Fiction", icon: BookOpen, count: "12.5k" },
  { name: "Non-Fiction", icon: BookOpen, count: "8.2k" },
  { name: "Science Fiction", icon: TrendingUp, count: "4.1k" },
  { name: "Mystery & Thriller", icon: Award, count: "6.3k" },
  { name: "Biography", icon: User, count: "3.8k" },
  { name: "Self-Help", icon: Star, count: "5.4k" },
];
...
export default function Home() {
  ...
  const handleGetRecommendations = () => {
    ...
    setTimeout(() => {
      // Get 3 random books that aren't in the featured list
      const availableBooks = BOOKS.filter(b => !FEATURED_BOOKS.find(fb => fb.id === b.id));
    ...
    <div className="bg-white dark:bg-gray-950 transition-colors duration-200">
      ...
      <div className="max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-end mb-8">
          <div>
            ...
            <p className="mt-1 text-gray-500 dark:text-gray-400">Find books in your favorite genres</p>
          </div>
          ...
          {CATEGORIES.map((category) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white text-center">{category.name}</h3>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{category.count} books</p>
              </Link>
            );
          })}
        </div>
      ...
      <div className="bg-gray-50 dark:bg-gray-900 py-16 transition-colors duration-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-end mb-8">
            <div>
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Bestselling Books</h2>
              <p className="mt-1 text-gray-500 dark:text-gray-400">The most popular books this week</p>
            </div>
            <Link to="/catalog?sort=popular" className="hidden sm:flex items-center text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 font-medium transition-colors">
              ...
              <div key={book.id} className="group flex flex-col bg-white dark:bg-gray-950 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow border border-gray-100 dark:border-gray-800 relative">
                ...
                <button 
                  onClick={(e) => {
                    ...
                    if (isInWishlist(book.id)) {
                      ...
                      // @ts-ignore - FEATURED_BOOKS doesn't have format, but Book type requires it. It's fine for demo.
                ...
                <div className="p-5 flex flex-col flex-grow">
                  <div className="flex justify-between items-start mb-2">
                    <p className="text-xs font-medium text-indigo-600 dark:text-indigo-400 uppercase tracking-wider">{book.category}</p>
        ...
        <div className="bg-gradient-to-r from-indigo-50 dark:from-indigo-900/20 to-purple-50 dark:to-purple-900/20 rounded-3xl p-8 sm:p-12 flex flex-col md:flex-row items-center justify-between border border-indigo-100 dark:border-indigo-900/30">
          <div className="md:w-1/2 mb-8 md:mb-0 md:pr-8">
            ...
            <p className="text-lg text-gray-600 dark:text-gray-300 mb-6">
              Tell us what you like, and our AI will find the perfect next read for you. We analyze millions of data points to suggest books you'll love.
            </p>
            <button 
                ...
                {recommendations.map((book) => (
                  <Link key={book.id} to={`/book/${book.id}`} className="group flex flex-col bg-white dark:bg-gray-900 rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-all border border-gray-100 dark:border-gray-800">
                    ...
                    <div className="p-3 flex flex-col flex-grow">
                      <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-1 line-clamp-1 group-hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">{book.title}</h3>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mb-2 line-clamp-1">{book.author}</p>
                      <div className="mt-auto flex items-center justify-between">
                        <span className="text-sm font-bold text-indigo-600 dark:text-indigo-400">${book.price}</span>
                        <div className="flex items-center">
